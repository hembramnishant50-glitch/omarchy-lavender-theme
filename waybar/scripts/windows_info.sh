...
is_browser() {
  case "$1" in
    firefox*|Firefox*|firefoxpwa*|FirefoxPWA*|tor-browser*|torbrowser*|TorBrowser*|google-chrome*|chromium*|Chromium*|brave*|Brave*|edge*|microsoft-edge*|vivaldi*|Vivaldi*|opera*|Opera*|slimjet*|Slimjet*|yandex-browser*|Yandex*|arc*|Arc*|epiphany*|org.gnome.Epiphany*)
      return 0 ;;
    *) return 1 ;;
  esac
}
...
strip_browser_suffix() {
  local t="$1"
  t=$(echo "$t" | sed -E 's/\s*[—–\-]\s*(Mozilla )?Firefox$//')
  t=$(echo "$t" | sed -E 's/\s*[—–\-]\s*Google Chrome$//')
  t=$(echo "$t" | sed -E 's/\s*[—–\-]\s*Chromium$//')
  t=$(echo "$t" | sed -E 's/\s*[—–\-]\s*Brave$//')
  t=$(echo "$t" | sed -E 's/\s*[—–\-]\s*(Microsoft )?Edge$//')
  t=$(echo "$t" | sed -E 's/\s*[—–\-]\s*Vivaldi$//')
  t=$(echo "$t" | sed -E 's/\s*[—–\-]\s*Opera$//')
  t=$(echo "$t" | sed -E 's/\s*[—–\-]\s*Tor Browser$//')
  t=$(echo "$t" | sed -E 's/\s*[—–\-]\s*Yandex$//')
  t=$(echo "$t" | sed -E 's/\s*[—–\-]\s*Arc$//')
  t=$(echo "$t" | sed -E 's/\s*[—–\-]\s*Epiphany$//')
  echo "$t"
}
...
if is_browser "$CLASS"; then
  ...
  [ -z "$RAW_NAME" ] && RAW_NAME="$SHORT_CLASS"
